          ...
          orders.data.map((order) => (
            <div key={order.id} className="bg-white shadow rounded-md mb-6 p-4">
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-2 border-b border-gray-200 pb-1 text-sm text-gray-700">
                ...
                <div className="font-semibold text-gray-900">${Number(order.total_price).toFixed(2)}</div>
              </div>

              {/* Vendor Info */}
              <div className="mb-3 text-sm text-gray-600">
                <div><span className="font-semibold">Vendor:</span> {order.vendorUser.name}</div>
                <div><span className="font-semibold">Store:</span> {order.vendorUser.store_name}</div>
                <div><span className="font-semibold">Address:</span> {order.vendorUser.store_address}</div>
              </div>
                  ...
                  {order.orderItems.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      <td className="border p-2 flex items-center gap-2">
                        ...
                        />
                        <Link href={`/product/${item.product.id}`} className="text-gray-800 hover:underline truncate max-w-xs">
                          {item.product.title}
                        </Link>
                      </td>
